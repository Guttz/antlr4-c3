...
import { FlinkSqlParser } from "./generated/FlinkSqlParser";
import { FlinkSqlLexer } from "./generated/FlinkSqlLexer";
import { SimpleGrammarLexer } from "./generated/SimpleGrammarLexer";
import { SimpleGrammarParser } from "./generated/SimpleGrammarParser";

export class TestErrorListener extends BaseErrorListener<ATNSimulator> {
  public errorCount = 0;

  public override syntaxError<T extends Token>(
...
describe("Code Completion Tests", () => {
  describe("Whitebox grammar tests:", () => {
    ...
    it.each(["test4", "test5", "test6", "test7"])(
      "Caret at one of multiple possible states (%s)",
      (test) => {
        ...
        const ctx = (
          parser[test as keyof WhiteboxParser] as () => ParserRuleContext
        )();
  ...
  describe("C++14 parser:", () => {
    it("Simple C++ example", () => {
      ...
      expect(
        candidates.rules.get(CPP14Parser.RULE_namespacename)?.ruleList
      ).toEqual([
      ...
      //Same as above
      expect(
        candidates.rules.get(CPP14Parser.RULE_classname)?.ruleList
      ).toEqual([
        CPP14Parser.RULE_translationunit,
        CPP14Parser.RULE_declarationseq,
        CPP14Parser.RULE_declaration,
      ...
      expect(candidates.rules.size).toEqual(3);
      expect(
        candidates.rules.get(CPP14Parser.RULE_idexpression)?.ruleList
      ).toEqual(idexpressionStack);
      expect(
        candidates.rules.get(CPP14Parser.RULE_classname)?.ruleList
      ).toEqual([
  ...
  describe("Simple expression parser:", () => {
    ...
    it("Typical setup", () => {
      ...
      expect(candidates.rules.size).toEqual(2);
      expect(
        candidates.rules.get(ExprParser.RULE_functionRef)?.startTokenIndex
      ).toEqual(6);
      expect(
    ...
    it("Recursive preferred rule", () => {
      ...
      expect(
        candidates.rules.get(ExprParser.RULE_simpleExpression)?.startTokenIndex
      ).toEqual(6);
    ...
    it("Candidate rules with different start tokens", () => {
      ...
      expect(
        candidates.rules.get(ExprParser.RULE_assignment)?.startTokenIndex
      ).toEqual(0);
      expect(
        candidates.rules.get(ExprParser.RULE_variableRef)?.startTokenIndex
      ).toEqual(0);
  ...
  describe("Flink sql", () => {
    ...
    it("actual useful setup without filters", () => {
      // No customization happens here, so the c3 engine only returns lexer tokens.
      const inputStream = CharStreams.fromString("SHOW CURRENT;");
      const lexer = new FlinkSqlLexer(inputStream);
      ...
      //const candidates = core.collectCandidates(0);
      //expect(candidates.tokens.size).toEqual(21);
      ...
      tokenStream.getTokens().forEach((token, index) => {
        ...
        const allValidTokens = [];
        for (const key of candidates.tokens.keys()) {
          console.log("=====> getDisplayName: ", key);
          allValidTokens.push(parser.getVocabulary().getDisplayName(key));
        }
    ...
    describe("SHOW test", () => {
      it("at position 5", () => {
        ...
        const candidates = core.collectCandidates(5);
    ...
    describe("USE test", () => {
      it("at position 1", () => {
        // No customization happens here, so the c3 engine only returns lexer tokens.
        const inputStream = CharStreams.fromString("USE DATABASE_X");
        const lexer = new FlinkSqlLexer(inputStream);
  ...
  describe("Simple Grammar", () => {
    it("simplest setup position 0", () => {
      // No customization happens here, so the c3 engine only returns lexer tokens.
      const inputStream = CharStreams.fromString("USE");
    ...
    it("simplest setup position 1", () => {
      // No customization happens here, so the c3 engine only returns lexer tokens.
      const inputStream = CharStreams.fromString("USE CATALOG");
      const lexer = new SimpleGrammarLexer(inputStream);
