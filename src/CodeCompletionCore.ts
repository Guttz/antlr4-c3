...
export class CodeCompletionCore {
  ...
  private checkPredicate(transition: PredicateTransition): boolean {
    return transition
      .getPredicate()
      .evaluate(this.parser, ParserRuleContext.EMPTY);
  }
  ...
  private translateStackToRuleIndex(
    ruleWithStartTokenList: RuleWithStartTokenList
  ): boolean {
  ...
  private translateToRuleIndex(
    ...
    if (this.preferredRules.has(ruleIndex)) {
      ...
      const path = ruleWithStartTokenList
        .slice(0, i)
        .map(({ ruleIndex: candidate }) => {
          return candidate;
        });
      let addNew = true;
      for (const rule of this.candidates.rules) {
  ...
  private determineFollowSets(
    ...
    const isExhaustive = this.collectFollowSets(
      start,
      stop,
      sets,
      stateStack,
  ...
  private processRule(
    ...
    let setsPerState = CodeCompletionCore.followSetsByATN.get(
      this.parser.constructor.name
    );
    if (!setsPerState) {
      setsPerState = new Map();
      CodeCompletionCore.followSetsByATN.set(
        this.parser.constructor.name,
        setsPerState
      );
    }
          ...
          case Transition.WILDCARD: {
            if (atCaret) {
              if (!this.translateStackToRuleIndex(callStack)) {
                for (const token of IntervalSet.of(
                  Token.MIN_USER_TOKEN_TYPE,
                  this.atn.maxTokenType
                ).toArray()) {
  ...
  private generateBaseDescription(state: ATNState): string {
    const stateValue =
      state.stateNumber === ATNState.INVALID_STATE_NUMBER
        ? "Invalid"
        : state.stateNumber;

    return (
      `[${stateValue} ${
        CodeCompletionCore.atnStateTypeMap[state.stateType!]
      }] in ` + `${this.ruleNames[state.ruleIndex]}`
    );
  }
  ...
  private printDescription(
    ...
    if (this.debugOutputWithTransitions) {
      for (const transition of state.transitions) {
        let labels = "";
        const symbols: number[] = transition.label
        ...
        transitionDescription +=
          ...
          `${
            CodeCompletionCore.atnStateTypeMap[transition.target.stateType!]
          }] in ` +
